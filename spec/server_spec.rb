  ...
  describe Configuration do
    ...
    describe '#initialize' do
      ...
      context 'when do not have credentials or port' do
        let(:server) { Configuration.new('www.outpost.test') }
      ...
      context 'with user credentials' do
        let(:server) { Configuration.new('batman@www.outpost.test') }
      ...
      context 'with port and without a user' do
        let(:server) { Configuration.new('www.outpost.test:5080') }
      ...
      context 'with user credentials and port' do
        let(:server) { Configuration.new('joker@www.gotham.test:8080')}
      ...
      context 'with user as option' do
        let(:server) { Configuration.new('www.gotham.city', :user => 'harlequina')}
      ...
      context 'with port as option' do
        let(:server) { Configuration.new('gotham.city', :port => 3000)}
